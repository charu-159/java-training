package leetcode;
class maxAmount{
    public int maximumAmount(int[][] coins) {
        int m = coins.length, n = coins[0].length;
        // dp[i][j][k] = max coins at (i,j) with k neutralizations used
        int[][][] dp = new int[m][n][3];

        // Initialize with very small values
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < 3; k++) {
                    dp[i][j][k] = Integer.MIN_VALUE / 2;
                }
            }
        }

        // Starting cell
        for (int k = 0; k < 3; k++) {
            if (coins[0][0] >= 0) {
                dp[0][0][k] = coins[0][0];
            } else {
                if (k > 0) dp[0][0][k] = 0; // neutralize robber
                dp[0][0][k] = Math.max(dp[0][0][k], -coins[0][0]); // without neutralization
            }
        }

        // Fill DP table
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < 3; k++) {
                    if (i == 0 && j == 0) continue;

                    int val = coins[i][j];

                    // From top
                    if (i > 0) {
                        if (val >= 0) {
                            dp[i][j][k] = Math.max(dp[i][j][k], dp[i-1][j][k] + val);
                        } else {
                            dp[i][j][k] = Math.max(dp[i][j][k], dp[i-1][j][k] + val);
                            if (k > 0) dp[i][j][k] = Math.max(dp[i][j][k], dp[i-1][j][k-1]);
                        }
                    }

                    // From left
                    if (j > 0) {
                        if (val >= 0) {
                            dp[i][j][k] = Math.max(dp[i][j][k], dp[i][j-1][k] + val);
                        } else {
                            dp[i][j][k] = Math.max(dp[i][j][k], dp[i][j-1][k] + val);
                            if (k > 0) dp[i][j][k] = Math.max(dp[i][j][k], dp[i][j-1][k-1]);
                        }
                    }
                }
            }
        }

        // Final answer
        return Math.max(dp[m-1][n-1][0], Math.max(dp[m-1][n-1][1], dp[m-1][n-1][2]));
    }
}